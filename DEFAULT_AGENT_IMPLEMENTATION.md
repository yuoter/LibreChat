...
export const agentsEndpointSchema = z.object({
  ...
  defaultAgent: z.string().optional(),  // NEW - applies only to USER role
});
```
...
const injectDefaultAgent = async (req, res, next) => {
  try {
    ...
    if (endpoint === 'agents' && !agent_id) {
      // NOTE: req.user is available because this runs AFTER requireJwtAuth
      ...
      const agentExists = await getAgent({ agent_id: defaultAgentId });
      if (agentExists) {
        req.body.agent_id = defaultAgentId;
      } else {
        console.warn(`Default agent ${defaultAgentId} not found in database`);
      }
    }

    next();
  } catch (error) {
    console.error('Error in injectDefaultAgent middleware:', error);
    next(); // Continue without injecting default
  }
};
...
5. **Null Safety** - Use optional chaining throughout to handle cases where:
   - Agents endpoint is not configured
   - User role is not available
   - Config objects are missing
